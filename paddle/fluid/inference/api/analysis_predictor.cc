...
void UpdatePrivateDeviceContext(InferGPUContext *gpu_context,
  ...
  VLOG(4) << "thread id is " << std::this_thread::get_id() << ", stream id is "
...
void AnalysisPredictor::ClearExtraParams() {
  ...
  VLOG(4) << "Clear " << extra_params.size() << " extra params.";
}
    ...
    for (auto var : op->results()) {
      ...
      if (is_persistable && is_persistable.data()) {
        if (op->isa<::pir::ParameterOp>()) {
          var_name = op->dyn_cast<::pir::ParameterOp>().param_name();
        } else if (op->isa<paddle::dialect::DataOp>()) {
          var_name = op->attribute<pir::StrAttribute>("name").AsString();
        }
